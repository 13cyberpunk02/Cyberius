    ...
    ILogger<MinioStorageService> logger) : IStorageService
{
    ...
    public async Task<IReadOnlyList<string>> ListAllObjectNamesAsync(CancellationToken ct = default)
    {
        ...
        var objects = minioClient.ListObjectsEnumAsync(listArgs, ct);
    ...
    public async Task<TimeSpan> GetObjectAgeAsync(string objectName, CancellationToken ct = default)
    {
        try
        {
            ...
            return DateTime.UtcNow - stat.LastModified.ToUniversalTime();
        }
        catch
        {
            // Если не можем получить метаданные — считаем файл старым
